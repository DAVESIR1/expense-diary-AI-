...
export const MessageCategorizer = {
  ...
  process(sender: string, body: string, existingTransactions: Transaction[] = []): FinalExtractionResult {
    ...
    const userCategory = CategoryRuleEngine.evaluate(body, sender);
    const hasUserRule = typeof userCategory === 'string' && userCategory !== 'Other' && userCategory.trim().length > 0;
    ...
    if (ruleResult && ruleResult.amount) {
      ...
      const finalCategory = hasUserRule ? userCategory : mappedCat;
    ...
    if (parsedTxn && parsedTxn.amount) {
      const finalCategory = hasUserRule ? userCategory : parsedTxn.category;
